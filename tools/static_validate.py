...
for engine_name in ['SpectralCorpseEngine','RitualFMEngine','BoneResonatorEngine','WraithBreathEngine','ReliquaryEngine','CoffinBodyEngine','ChoirBodyEngine']:
...
require(all(x in cmake for x in ['WraithBreathEngine.cpp','ReliquaryEngine.cpp','CoffinBodyEngine.cpp','ChoirBodyEngine.cpp']),'bestiary engines are not all compiled')
require('HorrorCastleCoffinCheck' in cmake and 'HorrorCastleChoirCheck' in cmake,'COFFIN/CHOIR dedicated gates are not compiled')
require((root/'tools/CoffinCheck.cpp').exists() and (root/'tools/ChoirCheck.cpp').exists(),'missing Chamber VI acoustic regression checks')
coffin=text('tools/CoffinCheck.cpp'); choir=text('tools/ChoirCheck.cpp')
require(all(term in coffin for term in ['COFFIN LID','COFFIN DREAD','COFFIN expression','extreme coupling']),'COFFIN gate does not cover geometry, DREAD, expression and safety')
require(all(term in choir for term in ['CHOIR VOWEL','CHOIR AETHER','CHOIR expression','extreme coupling']),'CHOIR gate does not cover tract geometry, AETHER, expression and safety')
