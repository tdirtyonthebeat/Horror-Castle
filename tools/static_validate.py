...
for f in ['PossessionEngine.cpp','PossessionEngine.h','RitualsEngine.cpp','RitualsEngine.h','Grimoire.cpp','Grimoire.h','GraveChamber.cpp']: require((root/'Source/HorrorCastle'/f).exists(),f'missing {f}')
...
require('Destinations = 22' in curse,'HEX 2.0 destination count is not 22');require('busHex[19]' in engine and 'busHex[20]' in engine and 'busHex[21]' in engine,'new HEX destinations are not consumed');require('Grimoire::CurrentStateVersion' in processor,'versioned state marker missing');require('juce::Reverb' not in engine,'stock juce::Reverb returned to active engine');require('process(m, performanceMidi' in engine,'RITUALS MIDI path missing')
...
print('PASS  v1.3 version metadata + compatibility architecture')
print('PASS  Chamber VIII-X promoted append-only at indices 15-17')
print('PASS  ABYSS/MIRROR/POLTERGEIST/AURORA/VORTEX/SIREN live in Castle voice runtime')
print('PASS  host-automatable Nervous System enable/depth parameters')
print('PASS  Creature Routing Matrix processes previous physical snapshot per voice')
print('PASS  VORTEX EVENT -> SIREN pressure and POLTERGEIST -> AURORA field ecology')
