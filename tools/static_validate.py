...
for engine_name in ['SpectralCorpseEngine','RitualFMEngine','BoneResonatorEngine','WraithBreathEngine','ReliquaryEngine','CoffinBodyEngine']:
...
require('CoffinBodyEngine.cpp' in cmake and 'HorrorCastleCoffinCheck' in cmake,'COFFIN prototype or dedicated gate is not compiled')
require((root/'tools/CoffinCheck.cpp').exists(),'missing COFFIN acoustic regression check')
...
require(all(name in living for name in ['RitualFMEngine','BoneResonatorEngine','WraithBreathEngine','ReliquaryEngine']),'Living Engines gate does not cover all active engine families')
coffin=text('tools/CoffinCheck.cpp')
...
print('PASS  COFFIN nonlinear enclosure prototype + dedicated regression gate')
