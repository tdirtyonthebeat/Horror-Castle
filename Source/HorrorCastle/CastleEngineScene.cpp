    ...
    if(gen.type>=GeneratorType::ChamberI){
        ...
        if(isCrypt){
            switch(gen.type){
                ...
                case GeneratorType::ChamberIII: { // BONE RESONATOR 2.0 — modal physical body
                    ...
                    return boneResonator.renderSample(v.cryptBone[(size_t)slot],
                                                      freq, sh, character, expression,
                                                      v.velocity, sr);
                }
