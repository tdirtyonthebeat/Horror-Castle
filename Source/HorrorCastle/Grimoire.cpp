...
std::vector<Grimoire::SpellInfo> Grimoire::getFactorySpells() const
{
    ...
    return {
        ...
        S{"Black Weather","The storm learned your name","VORTEX collapse pulses force the SIREN plenum across its operating edge.","LIVING ECOLOGY","VORTEX • SIREN • PHYSICAL",40},
        S{"Static Halo","The dead charge the sky","POLTERGEIST arcs destabilize AURORA's shared field and ring alignment.","LIVING ECOLOGY","POLTERGEIST • AURORA • ELECTRIC",41},
        S{"The Castle Breathes","Every room has a pulse","Both ecology pairs run together: fluid collapse and electromagnetic haunting share one performance.","LIVING ECOLOGY","NERVOUS SYSTEM • DUAL • EXPRESSIVE",42},
        S{"Mirror in the Flood","The reflection drowns first","ABYSS and MIRROR form the body while VORTEX and SIREN animate the surrounding weather.","LIVING ECOLOGY","ABYSS • MIRROR • WEATHER",43},
    };
}
...
bool Grimoire::loadFactory(int index)
{
    ...
    switch(index) {
        ...
        case 42:set("crypt.g1.type",17.f);set("crypt.g1.level",.56f);set("crypt.g1.shape",.84f);set("crypt.g2.type",16.f);set("crypt.g2.level",.38f);set("crypt.g2.shape",.74f);set("tower.g1.type",17.f);set("tower.g1.level",.52f);set("tower.g1.shape",.48f);set("tower.g2.type",16.f);set("tower.g2.level",.40f);set("tower.g2.shape",.68f);set("ecology.enabled",1.f);set("ecology.depth",.76f);set("ritual.mode",3.f);set("ritual.mix",.30f);set("grave.reverb",.48f);break;
        case 43:set("crypt.g1.type",15.f);set("crypt.g1.level",.58f);set("crypt.g1.shape",.78f);set("crypt.g2.type",17.f);set("crypt.g2.level",.34f);set("tower.g1.type",15.f);set("tower.g1.level",.54f);set("tower.g1.shape",.70f);set("tower.g2.type",17.f);set("tower.g2.level",.32f);set("ecology.enabled",1.f);set("ecology.depth",.62f);set("grave.reverb",.56f);set("grave.delay",.22f);break;
...
juce::File Grimoire::saveUserPreset(const juce::String& requestedName) const
{
    ...
    auto state = apvts.copyState(); state.setProperty("stateVersion", CurrentStateVersion, nullptr); state.setProperty("productVersion", "1.3.0", nullptr);
