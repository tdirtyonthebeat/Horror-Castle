...
inline juce::AudioProcessorValueTreeState::ParameterLayout createLayout()
{
    ...
    auto addFloat = [&l](juce::String id, juce::String name, float lo, float hi, float def) {
        l.add(std::make_unique<juce::AudioParameterFloat>(id, name, juce::NormalisableRange<float>(lo, hi), def));
    };
    auto addChoice = [&l](juce::String id, juce::String name, std::initializer_list<juce::String> choices, int def) {
        juce::StringArray a;
        for (auto& c : choices) a.add(c);
        l.add(std::make_unique<juce::AudioParameterChoice>(id, name, a, def));
    };
    auto addBool = [&l](juce::String id, juce::String name, bool def) {
        l.add(std::make_unique<juce::AudioParameterBool>(id, name, def));
    };
    ...
    for (auto* s : scenes)
    {
        for (int g = 0; g < 3; ++g)
        {
            ...
            const float defaultTune = (g == 0) ? 0.f
                                      : (isCrypt ? (g == 1 ? -12.f : -24.f)
                                                 : (g == 1 ? 7.f : 12.f));
        ...
        for (int f = 0; f < 2; ++f)
        {
            ...
            addChoice(p("type"), juce::String(s) + " Filter " + juce::String(f + 1) + " Type",
                       {"Low Pass", "High Pass", "Band Pass", "Notch", "Comb", "Formant", "Diode", "K35", "Shaper"}, defaultFilterType);
            const float defaultCutoff = cryptFilter ? (f == 0 ? .16f : .23f)
                                                    : (f == 0 ? .065f : .40f);
            const float defaultRes = cryptFilter ? .18f : .24f;
            const float defaultDrive = cryptFilter ? .14f : .06f;
        ...
        addFloat(scene(s, "character"), juce::String(s) + " Character", 0.f, 1.f,
                 juce::String(s) == "crypt" ? .72f : .70f);
    ...
    addFloat("ritual.mix", "Ritual Mix", 0.f, 1.f, .0f);
    addFloat("ritual.depth", "Ritual Depth", 0.f, 1.f, .35f);
    addFloat("ritual.drive", "Ritual Fury", 0.f, 1.f, .12f);
    addFloat("ritual.width", "Ritual Width", 0.f, 1.f, .55f);
    addFloat("ritual.feedback", "Ritual Feedback", 0.f, .92f, .28f);
    addFloat("grave.reverb", "Grave Reverb", 0.f, 1.f, .32f);
    addFloat("grave.delay", "Grave Delay", 0.f, 1.f, .18f);
    addFloat("grave.feedback", "Grave Feedback", 0.f, .95f, .28f);
    addFloat("grave.cutoff", "Grave Tone", .02f, .48f, .32f);
    addFloat("grave.output", "Grave Output", 0.f, 1.f, .78f);
    addFloat("global.glide", "Castle Glide", 0.f, 1.f, 0.f);
    ...
    addChoice("rituals.rate", "Rituals Rate", {"1/4", "1/8", "1/16", "1/32"}, 2);
    addFloat("rituals.bpm", "Rituals BPM", 30.f, 240.f, 120.f);
    addFloat("rituals.gate", "Rituals Gate", .05f, .98f, .62f);
    ...
    for (int i = 1; i <= 8; ++i)
    {
        ...
        addFloat(prefix + "decayTime", "Hex Curse " + juce::String(i) + " Decay Time", .05f, 8.f, 1.50f);
        addFloat(prefix + "madnessSteps", "Hex Curse " + juce::String(i) + " Madness Steps", 2.f, 32.f, 8.f);
        addFloat(prefix + "bloodHunger", "Hex Curse " + juce::String(i) + " Blood Hunger", 0.f, 1.f, .65f);
    }
    return l;
}
} // namespace param
