...
static GeneratorType gt(int v){ return static_cast<GeneratorType>(juce::jlimit(0,14,v)); }
...
void CastleEngine::setParameters(const juce::AudioProcessorValueTreeState& apvts){
    ...
    auto loadScene=[&](SceneArchitecture& s,const char* name){
        for(int i=0;i<3;++i){auto& g=s.voice.generators[i];g.type=gt(choice(param::id(name,i+1,"type"),14,(int)g.type));g.level=get(param::id(name,i+1,"level"),g.level);g.pan=get(param::id(name,i+1,"pan"),g.pan);g.tune=get(param::id(name,i+1,"tune"),g.tune);g.shape=get(param::id(name,i+1,"shape"),g.shape);g.spread=get(param::id(name,i+1,"spread"),g.spread);g.enabled=getb(param::id(name,i+1,"enabled"),g.enabled);}
